...
We here present a use case programmed in our version of the join calculus.
However, since we don't have a mature, compiled language, we will not focus
...
Due to the lack of data structure implementations and the general ineffeciency
of the implementation we only consider a scenario with one collecting node and
two sensors that run a predefined program.

This program simply measures the local temperature and reports it every $n$'th
time instant.  However, if the sensor registers a temperature above a given
threshold it will report it immediately.

\fixme{If time permits we will make an attempt at implementing the distributed queries.}
...
\fixme{Update to be time-join-aware}
...
\fixme{Update to current use case version}
...
\fixme{Should we include a program example directly?.. it gets quite big..}
\begin{verbatim}
