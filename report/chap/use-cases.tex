...
synchronus call is the incarnation of this facility in our case.
...
structure, because process migration in the join calculus happens only 
the initiative of the process, that is to migrate.
...
The interface of the query programs is more explicitly specified below.
\fixme{insert reference to specification}
...
sensor hardware, and the sensor-part of the query program migrates to
the sensor.
While the sensor is active and registered, any exchange of sensor data
happens only on the initiative of the sensor part of the query
program.
...
phase.
Depending on the running queries however, this may not be desirable,
...
Since everything is asynchronus, the server and the sensor may each
