...
We need the following structural equivalences in our semantics:
\begin{align*}
 \textsc{Str-Def}&&
...
when the time of the two communicating machines are synchronised.\fxfatal{But, isn't this the always the case in this semantics :o}
\fxfatal{We still need to specify that communication can't take place when one of
the locations are failed.}

As spawning a new location doesn't cause a new physical machine to spontaneusly materialise, we have modeled the distinction between locations that corresponds to physical machines and those that don't by the convention, that the first symbol in the location string \fixme{bedre ord end ``location string''?} denotes the physical machine, that hosts the given location. By this convention it is reasonable to demand, that locations that share a common prefix exchange messages reliably, as they should only be subjected to the same physical limits as whatever else runs on that given machine.
\begin{equation*}
...
\end{equation*}\fixme{Actually, it shouldn't be the powerset, but rather the set of subsets.}
\fixme{How do we state this in an elegant way?}
...
\end{equation*}
\fxfatal{For the time being we'll just let time pass here and allow
communication in to happen within an instance, partly because it seems simpler
to me, partly because it reflects what the code actually does, and partly
because I don't really get the motivation for forcing the communication to pile
up and happen at the end of an instance\ldots wall of text :o}
