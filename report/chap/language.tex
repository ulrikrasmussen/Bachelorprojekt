...
%\def\migrates(#1,#2){\stackrel{#1\mapsto#2}{\leadsto}}
...
\forall D \in \D ~.~ \Big( D \equiv J\toJ{d}Q \\
                         \lnot\exists P' \subseteq\A~.~( \M \time{t'} P' \\ t'+d\leq t \\ \P (P') \equiv \sigma_{dv}J)\Big)
}
{
  \quiet(\M)
}
...
\end{equation*}
\end{minipage}}
\caption{Inference rules for migration}
\end{figure}
