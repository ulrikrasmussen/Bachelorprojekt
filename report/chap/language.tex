...
\doms{$\mathcal{P}_J(J,P) \bowtie \hat\sigma$} \doms{$\mathcal{P}_\pi(\pi, v) \bowtie \hat\sigma$}
\begin{equation*}
\inferrule[Pat-Var]
{~}
{\mathcal{P}_\pi(x,v) \bowtie (x \mapsto v)}
\end{equation*}
\begin{equation*}
\inferrule[Pat-Cons]
{\pi = \kappa(\pi_1, \pi_2, ..., \pi_n)
\\ v = \kappa(v_1, v_2, ..., v_n)
\\ \mathcal{P}_\pi(\pi_1,v_1) \bowtie \sigma_1 \land \mathcal{P}_\pi(\pi_2,v_2)
   \bowtie \sigma_2 \land ... \land \mathcal{P}_\pi(\pi_n,v_n) \bowtie \sigma_n}
{
 \mathcal{P}_\pi(\pi,v) \bowtie \sigma_1 \cup \sigma_2 \cup ... \cup \sigma_n
}
\end{equation*}
\begin{equation*}
...
{\pi = \kappa(\pi_1, \pi_2, ..., \pi_n)
\\ v = \kappa'(v_1, v_2, ..., v_n)
\\ \kappa \not= \kappa' \lor \P_\pi(\pi_i, v_i) \bowtie fail
}
{
 \P_\pi(\pi, v) \bowtie fail
}
\end{equation*}
\begin{equation*}
\inferrule[Pat-Msg]
{J = x\langle \pi_1, \pi_2, ..., \pi_n \rangle
 ...
 \\ \mathcal{P}_\pi(\pi_1,v_1) \bowtie \sigma_1 \land \mathcal{P}_\pi(\pi_2,v_2)
    \bowtie \sigma_2 \land ... \land \mathcal{P}_\pi(\pi_n,v_n) \bowtie \sigma_n}
{
 P_J(J,P) \bowtie \sigma_1 \cup \sigma_2 \cup ... \cup \sigma_n
}
\end{equation*}
\begin{equation*}
...
{J = x(\pi_1, \pi_2, ..., \pi_n)
\\ P = y(v_1, v_2, ..., v_n)
\\ x \not= y \lor \P_\pi(\pi_i, v_i) \bowtie fail
}
{
 \P_J(J, P) \bowtie fail
}
...
{J = J_1 ~\&~ J_2
\\ P = P_1 ~\&~ P_2
\\ \mathcal{P}_J(J_1,P_1) \bowtie \sigma_1
\\ \mathcal{P}_J(J_2,P_2) \bowtie \sigma_2
}
{\mathcal{P}_J(J,P) \bowtie \sigma_1 \cup \sigma_2}
\end{equation*}
\begin{equation*}
\inferrule[Pat-Par-Fail]
{J = J_1 ~\&~ J_2
\\ P = P_1 ~\&~ P_2
\\ \mathcal{P}_J(J_1,P_1) \bowtie fail
\lor \mathcal{P}_J(J_2,P_2) \bowtie fail
}
{\mathcal{P}_J(J,P) \bowtie fail}
\end{equation*}
\end{minipage}}
...
%auxiliary rules in Figure \ref{fig:rule:aux}.
%\begin{figure}[!h]
%\fbox{\begin{minipage}{0.97\textwidth}
%\doms{$\M\longrightarrow\M'$}
%\begin{align*}
%\inferrule[Local-Red]
%{ \mathcal{D}_1 \vdash_t^\phi \mathcal{A}_1
%  \longrightarrow \mathcal{D}_1' \vdash_t^\phi \mathcal{A}_1'}
...
%\end{equation*}
%\end{minipage}}
%\caption{Auxiliary rules to lighten the notation}\label{fig:rule:aux}
%\end{figure}
%
...
{
\M = \D \vdash_t^\phi \A
\\ \D = \{ a[D~\mathbf{in}~P] \} \uplus \D'
}
{
 \M \longrightarrow (D \vdash_t^{\phi a} P) \parallel (\D' \vdash_t^\phi \A)
}
\end{equation*}
\begin{equation*}
\inferrule[Red-React]
...
\begin{figure}[!h]
\fbox{\begin{minipage}{0.97\textwidth}
\doms{$\mathcal{C} \longrightarrow \mathcal{C}'$}
\begin{equation*}
\inferrule[Comm-Link]
{
...
\fixme{Well, everything is work in progress currently, but this section is even more so..}

\doms{$R_a( J\toJ d P,\; \A,\; \M)$}
...
\doms{$Q(\M)$}
\begin{equation*}
\inferrule[No-JMatch-Empty]
{
}
{\emptyset \noMatch t J}
\qquad
\inferrule[No-JMatch-Single]
{
  P\equiv \atm t' : x <\tilde v>
  \\ P_J(J,\; P_\star ) = fail \lor t' \ge t
}
{
  \{| \atm x<\tilde v>|\} \noMatch t J
}
\end{equation*}\\
\begin{equation*}
\inferrule[No-JMatch-Recurse]
{
  \A = \{|P|\}\uplus \A'
  \\ P \equiv \atm t': x<\tilde v>
  \\ P_J(J,\; \atm x < \tilde v > ) = fail \lor t'\ge t
  \\ \A'\noMatch t J
}
{
  \A \noMatch t J
...
\end{equation*}\\
\begin{equation*}
\inferrule[R-Activate]
{
  \M \equiv J\toJ d Q \mscJ \A
}
{
  R_a( J \toJ d Q,\; \A, \M)
}\qquad
\inferrule[R-Succ-React]
{
  R_s(\M,\; \A')
  \\ \M \equiv \D\mscJ\A
}
{
  \M \longrightarrow \D\mscJ\A'
}
\end{equation*}\\
\begin{equation*}
\inferrule[R-Activate-Recurse]
{
  R_a(\atm\kappa<\tilde\pi> \andJ J \toJ d Q,\; \A,\; \D\mscJ\B)
  \\ \A = \{| P |\} \uplus \A'
  \\ P = t' : \atm x<\tilde v>
  \\ P_J(\atm\kappa<\tilde\pi>,\; \atm x<\tilde v>) = \sigma
  \\ t'+d\leq t
}
{
  R_a(J\toJ d \sigma Q,\; \A')
}
\end{equation*}\\
\begin{equation*}
\inferrule[R-Succ]
{
  R_a(\atm\kappa<\tilde\pi> \toJ d  Q,\; \A,\; \D\mscJ\B)
  \\ \A = \{| P |\} \uplus \A'
  \\ P = t' : \atm x<\tilde v>
  \\ P_J(\atm\kappa<\tilde\pi>, \atm x<\tilde v>) = \sigma
  \\ t'+d\leq t
